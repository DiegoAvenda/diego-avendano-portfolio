<script>
	...
	onMount(() => {
		...
		// Crear plano circular para la imagen del sol
		textureLoader.load('/diego.jpg', (texture) => {
			const sunMaterial = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide });
			const sunGeometry = new THREE.CircleGeometry(2, 32); // Usar CircleGeometry
